...
- 菜单栏弹窗展示主额度、Credits、使用限制重置次数、今日套餐消耗与数据来源状态；
- Credits 剩余额度按整数显示，避免长小数干扰菜单栏可读性；
- 今日用量优先显示本机结构化 `token_count` 的有效 Token（扣除缓存输入）；当天完全没有 Token 记录时，才根据 Credits 余额下降显示明确标注的估算值；
- 套餐剩余百分比和重置时间仍直接采用 OpenAI 服务端返回的限额，不用本机 Token 反推；
- 完整 Dashboard 提供总结、额度详情、使用历史、告警、数据来源和设置六个页面，并默认打开总结页；
...
- 支持最近 7 天逐日 Token、缺失日的 Credits 估算、Token 合计/日均/峰值和本机历史检测到的重置次数，不混合累加不同单位；
...
- 登录态、历史快照、诊断信息均保存在本机。
- 支持导出版本化 JSON、历史 CSV 与经过敏感信息脱敏的诊断 JSON。
...
| 5 | 本机用量事件 | 今日有效 Token、最近 7 天每日统计 | 读取 `token_count` 计数，不读取消息正文；无 Token 记录时才使用 Credits 余额变化估算 |
| 6 | 本机 Claude Code 用量 | 最近 7 天每日 Token | 用户单独授权；只读取结构化 usage 数值、消息 ID 与时间戳，不读取会话正文或认证凭据 |

额度、Credits、Analytics 和今日套餐消耗可以来自不同来源。应用会保留每个模块的来源与可信度，缺失字段显示为暂无数据，不用 `0` 伪装未知值。

## 完整面板

- **总结**：当前额度、消耗速度和最近 7 天每日 Token；缺失日单独显示 Credits 估算；
- **概览**：额度概览、套餐、重置窗口、趋势与刷新状态；
- **使用历史**：历史快照、趋势图、近期用量和重置记录；
- **告警**：额度阈值、用量提醒和通知状态；
- **数据来源**：本机 Codex、OpenAI 页面、缓存、估算与刷新诊断；
- **设置**：刷新频率、登录启动、隐私、历史保留和本地数据管理。
- **设置 / 隐私**：导出用量 JSON、历史 CSV 和脱敏诊断报告。

## 隐私说明

- 不上传登录态、Cookie、Token、Session ID、提示词、回复或代码内容；
- 本机每日统计只扫描 `~/.codex/sessions` 中最近 7 天的结构化 Token 计数、模型、速度和时间戳；Token 缺失时可根据真实 Credits 余额变化提供估算；
- Claude Code 本机统计只扫描 `~/.claude/projects` 中结构化用量字段、消息 ID 和时间戳，用于去重和按日汇总；
